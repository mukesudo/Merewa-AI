...
class LLMService:
    ...
    async def generate_voice_script(self, user_prompt: str, language: str) -> str:
        prompt = (
            "You are a creative social media assistant for Merewa, a voice-first social app.\n"
            "Write a short, engaging script (2-4 sentences) for a user to record as a voice post.\n"
            f"Language: {language}\n"
            f"Topic/Prompt: {user_prompt}\n\n"
            "Constraints:\n"
            "- Sound natural and conversational, as if spoken by a person.\n"
            "- If the language is 'am', you MUST write in Amharic using the Ge'ez script.\n"
    ...
    async def _generate(self, prompt: str, fallback: str) -> str:
        try:
            ...
            return await asyncio.to_thread(self._request_groq, prompt)
        except Exception:
            return fallback
    ...
    def _context_block(context: List[Dict[str, object]]) -> str:
        ...
        for index, item in enumerate(context, start=1):
            lines.append(f"{index}. @{item['author']} ({item['language']}): {item['content']}")
        return "\n".join(lines)
